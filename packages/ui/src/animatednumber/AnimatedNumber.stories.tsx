...
const meta: Meta<typeof AnimatedNumber> = {
  ...
  argTypes: {
    ...
    weight: { control: 'select', options: ['bold', 'semibold', 'medium', 'regular', 'light'] },
    duration: { control: { type: 'range', min: 300, max: 3000, step: 100 } },
    prefix: { control: 'text' },
    suffix: { control: 'text' },
    decimals: { control: { type: 'number', min: 0, max: 4 } },
    easing: { control: 'inline-radio', options: ['linear', 'easeOut', 'easeInOut'] },
  },
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', alignItems: 'center' }}>
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <button style={{ padding: '0.375rem 0.75rem', cursor: 'pointer' }} onClick={() => setVal(Math.floor(Math.random() * 50000))}>
            Randomize Value
          </button>
          <button style={{ padding: '0.375rem 0.75rem', cursor: 'pointer' }} onClick={() => setVal(0)}>
            Reset to 0
          </button>
        </div>
