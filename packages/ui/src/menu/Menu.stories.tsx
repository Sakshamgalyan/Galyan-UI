...
export const WithCustomHeader: Story = {
  render: () => {
    const [active, setActive] = useState('all-users');
    return (
      <Menu variant="bordered" items={menuItems} activeItemId={active} onItemClick={setActive}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#22c55e', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold' }}>
            SG
          </div>
          <div>
            <div style={{ fontWeight: 600, fontSize: '0.875rem' }}>Saksham Galyan</div>
            <div style={{ fontSize: '0.75rem', color: '#64748b' }}>Admin Workspace</div>
          </div>
        </div>
      </Menu>
    );
...
export const HorizontalMenu: Story = {
  decorators: [(Story) => <div style={{ width: '100%', maxWidth: 700 }}><Story /></div>],
  render: () => {
    const [active, setActive] = useState('dashboard');
...
export const Sizes: Story = {
  decorators: [(Story) => <div style={{ width: 320, display: 'flex', flexDirection: 'column', gap: '2rem' }}><Story /></div>],
  render: () => {
    const [active, setActive] = useState('users');
    return (
      <>
        <div>
          <div style={{ fontSize: '0.75rem', fontWeight: 600, color: '#64748b', marginBottom: '0.5rem' }}>Small Size (sm)</div>
          <Menu size="sm" variant="bordered" items={menuItems} activeItemId={active} onItemClick={setActive} />
        </div>
        <div>
          <div style={{ fontSize: '0.75rem', fontWeight: 600, color: '#64748b', marginBottom: '0.5rem' }}>Medium Size (md)</div>
          <Menu size="md" variant="bordered" items={menuItems} activeItemId={active} onItemClick={setActive} />
        </div>
        <div>
          <div style={{ fontSize: '0.75rem', fontWeight: 600, color: '#64748b', marginBottom: '0.5rem' }}>Large Size (lg)</div>
...
export const DisabledItems: Story = {
  render: () => {
    ...
    const itemsWithDisabled = [
      { id: 'dashboard', label: 'Dashboard', icon: <DashboardIcon /> },
      { id: 'analytics', label: 'Analytics (Disabled)', icon: <UsersIcon />, disabled: true },
      { id: 'settings', label: 'Settings', icon: <SettingsIcon /> },
    ];
