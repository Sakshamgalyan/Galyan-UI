...
export function ThemeProvider({
  ...
  useEffect(() => {
    try {
      ...
      if (stored) {
        const { role: r, colorMode: c } = JSON.parse(stored) as { role?: ThemeRole; colorMode?: ColorMode };
        if (r) setRoleState(r);
  ...
  const setRole = useCallback((newRole: ThemeRole) => {
    ...
    try {
      ...
      localStorage.setItem(storageKey, JSON.stringify({ ...current, role: newRole }));
    } catch { /* ignore */ }
  }, [storageKey]);
  ...
  const setColorMode = useCallback((newMode: ColorMode) => {
    ...
    try {
      const stored = localStorage.getItem(storageKey);
      const current = stored ? JSON.parse(stored) : {};
      localStorage.setItem(storageKey, JSON.stringify({ ...current, colorMode: newMode }));
    } catch { /* ignore */ }
  }, [storageKey]);
